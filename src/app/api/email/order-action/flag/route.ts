...
export async function GET(request: NextRequest) {
  try {
    ...
    if (payload.action !== "flag") {
      return new NextResponse(renderHtml("Invalid Action", "This link is not a flag link."), {
        status: 400,
        headers: { "Content-Type": "text/html; charset=utf-8" },
      });
    }
    ...
    return new NextResponse(
      renderHtml("Order Flagged", "Flag saved. You can now return to the Reconciliation screen."),
